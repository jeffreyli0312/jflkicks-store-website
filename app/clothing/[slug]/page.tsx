...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    if (!slug) return { title: "Clothing | JFLKicks" };

    const clothing = await sanity.fetch(
        `
    *[_type == "clothing" && slug.current == $slug][0]{
      title,
      description
    }
    `,
    ...
    if (!clothing) return { title: "Item Not Found | JFLKicks" };
...
export default async function ClothingPage({ params }: Props) {
    const { slug } = await Promise.resolve(params);
    if (!slug) return <div className="p-8">Missing clothing slug</div>;
    ...
    *[_type == "clothing" && slug.current == $slug][0]{
    ...
    if (!clothing) return <div className="p-8">Item not found</div>;
                        ...
                        <div className="grid gap-10 lg:grid-cols-2 lg:items-start">
                            ...
                            <div className="lg:sticky lg:top-24">
                                <div className="mx-auto w-full max-w-2xl">
                                    {Array.isArray(clothing.images) && clothing.images.length > 0 && (
                                        <ProductCarousel images={clothing.images} title={clothing.title} />
                                    )}
                                </div>
                            </div>
